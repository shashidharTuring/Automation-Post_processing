    ...
    if json_file:
        with st.spinner("Loading and analyzing JSON..."):
            ...
            if json_data:
                ...
                #     client = openai.OpenAI(api_key=OPENAI_API_KEY)
                if st.button("🧠 Analyze with AI", help="Get AI-powered insights about this JSON structure"):
                    # Prefer Streamlit secrets, fallback to environment variable if present
                    OPENAI_API_KEY = st.secrets.get("OPENAI_API_KEY") or os.getenv("OPENAI_API_KEY")
                    if not OPENAI_API_KEY:
                        st.error("❌ OPENAI_API_KEY missing. Add it to `.streamlit/secrets.toml` or as an environment variable.")
                        st.stop()
